...
df <- fread("C:/Users/Documents/R/uconn_comp_2018_train.csv/uconn_comp_2018_train.csv") %>% 
  filter(!fraud == -1)
df_test <- fread("C:/Users/Documents/R/uconn_comp_2018_train.csv/uconn_comp_2018_test.csv")
...
df = transform(df, claim_est_payout = ifelse(is.na(claim_est_payout), mean(claim_est_payout, na.rm=TRUE), claim_est_payout))
df_train <- df_4[index,]
df_test <- df_4[-index,]
